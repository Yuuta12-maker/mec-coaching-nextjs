...
async function getAuthClient() {
  try {
    ...
    try {
      // 安全にキーをパースするための事前処理
      if (typeof serviceAccountKeyStr === 'string') {
        // サービスアカウントキーが有効なJSONか確認
        const trimmedKey = serviceAccountKeyStr.trim();
        ...
        // すでにオブジェクトならそのまま使用
        parsedKey = serviceAccountKeyStr;
      }
      
      // 必要なプロパティがあるか確認
      if (!parsedKey.client_email || !parsedKey.private_key) {
        throw new Error('サービスアカウントキーに必要な情報が含まれていません');
      }
      
      logger.info('メール送信用サービスアカウントキーのパース成功:', { email: parsedKey.client_email });
    } catch (parseError) {
      logger.error('JSONパースエラー:', parseError);
      throw new Error(`サービスアカウントキーのJSONパースに失敗しました: ${parseError.message}`);
    }
    
    // 修正点: メール送信のために Gmail API スコープを追加
