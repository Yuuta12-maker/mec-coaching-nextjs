...
export async function fetchData(endpoint, options = {}) {
  try {
    console.log(`API呼び出し: ${endpoint} 開始`);
    const response = await fetch(`/api/${endpoint}`, options);
    
    if (!response.ok) {
      const errorData = await response.json();
      console.error(`API呼び出しエラー (${endpoint}):`, errorData);
      throw new Error(errorData.error || `API呼び出しエラー: ${response.status}`);
    }
    
    const data = await response.json();
    console.log(`API呼び出し: ${endpoint} 成功`, data);
    return data;
  } catch (error) {
...
export async function getPayments() {
  try {
    ...
    console.log('支払いデータ取得結果:', response);
    
    // 支払いデータをログに出力（デバッグ用）
    if (response.payments && response.payments.length > 0) {
      console.log('最初の支払いデータ:', response.payments[0]);
      console.log('支払いステータスの種類:', [...new Set(response.payments.map(p => p.状態))]);
    }
