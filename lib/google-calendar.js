...
const createCalendarClient = () => {
  try {
    ...
    try {
      ...
      logger.info('サービスアカウントキープレビュー:', keyPreview);
      
      // キーがオブジェクトなのか文字列なのか確認
      ...
      logger.info('サービスアカウントキーのパース成功:', { email: credentials.client_email });
    } catch (parseError) {
      logger.error('GOOGLE_SERVICE_ACCOUNT_KEYのParseエラー:', parseError);
      throw new Error(`サービスアカウントキーが正しいJSON形式ではありません: ${parseError.message}`);
    }
